...
var just_env = 'prod'; 
var __TOP_DOMAIN__ = (/^((?:(?:25[0-5]|2[0-4]\d|(?:1\d{2}|[1-9]?\d))\.){3}(?:25[0-5]|2[0-4]\d|(?:1\d{2}|[1-9]?\d)))$/).test(document.domain)
    ? document.domain
    : document.domain.split('.').slice(-2).join('.');

// 清除子域名下的Cookie参数
if (__TOP_DOMAIN__ !== document.domain) {
    Cookies.remove('token');
    Cookies.remove('userId');
}

// 用户是否已登录状态全局变量
window.__ABC_SSO_IS_AUTHORIZED__ = false;
...
switch(just_env){
    case 'local' : 
    case 'dev' : 
        sso_baseUrl = 'https://passport-dev.abcfintech.com';
        break;
    case 'pre' : 
...
function getBackUrl() {
    ...
    if(local.hash){
        ...
        // 链接已经存在ticket + userid + skipAuth 需要去掉
...
function sso_getTokenByTicket() {
    ...
    if (resData.errorCode != 511 && resData.errorCode != 41) {
        Cookies.remove('token', { domain: __TOP_DOMAIN__ });
        Cookies.remove('userId', { domain: __TOP_DOMAIN__ });
        Cookies.set('token', resData.data, { expires: 30, domain: __TOP_DOMAIN__ })
        Cookies.set('userId', sso_getQueryString('userId'), { expires: 30, domain: __TOP_DOMAIN__ })

        // 标记用户为已登陆状态
        window.__ABC_SSO_IS_AUTHORIZED__ = true;
    }else{
        sso_checkToken();
    }
}
...
function sso_tologin(isImmediate) {
    // 标记用户为未登陆状态
    window.__ABC_SSO_IS_AUTHORIZED__ = false;

    // 检查当前路由是否存在于免登陆的路由白名单中
    var routePath = window.location.pathname;
    var isSkipAuth = window.__ABC_SSO_SKIP_AUTH_WHITELIST__
        && window.__ABC_SSO_SKIP_AUTH_WHITELIST__.some(function (route) {
    ...
    // 如果存在免登陆参数，且非直接跳转到登录页，将不做任何处理
    if (isSkipAuth && !isImmediate) {
        return;
    }

    // 如果非直接跳转到登录页，将先跳转到运营页面
    if (!isImmediate) {
